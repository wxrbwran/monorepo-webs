...
function CertificateEdit(props: IProps) {
  ...
  const removeImage = (key: string, imgUrl: string, imgIndex: number) => {
    const newCertificates: CommonData = { ...getFieldValue('certificates') };
    newCertificates[key] = newCertificates[key].filter((img: any) => img !== imgUrl);
    setFieldsValue({
      certificates: newCertificates,
    });
  ...
  const handleShowViewer = (imgUrl: string, keyName: string, imgIndex: number) => {
    setShowViewer(true);
    setShowImg({
      imgUrl,
      keyName,
  ...
  const handleImageRotate = (degreeNumber: number) => {
    const degree: number = degreeNumber < 0
      // eslint-disable-next-line radix
      ? (degreeNumber + (parseInt((Math.abs(degreeNumber) / 360).toString()) + 1) * 360)
      : degreeNumber;
            ...
            <div className={styles.img_wrap}>
              {
                certificateArr[item].map((imgUrl: string, imgIndex: number) => {
                  ...
                  return (
                    <div
                      key={imgUrl}
                      className={defaultImg.includes(imgUrl) ? styles.upload : styles.has_img}
                    >
                      ...
                      <div className={styles.action}>
                        ...
                        <EyeOutlined onClick={() => handleShowViewer(imgUrl, item, imgIndex)} />
                      </div>
