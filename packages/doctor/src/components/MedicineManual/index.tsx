...
function MedicineManual({ children, medicineId, medicineType }: IProps) {
  ...
  const fetchManual = () => {
    ...
    api.medicine.getManual(params).then((res) => {
      setManual(res);
      setIsLoading(false);
      if (res) {
        setIsHave(true);
  ...
  return (
    <>
      <div onClick={medicineType === 2 ? () => {} : fetchManual} className={styles.manual_pic}>
        {children}
      </div>
      {showModal && (
      <DragModal
      ...
      >
        <div>
          {isLoading ? <LoadingOutlined /> : medicineManual}
        </div>
      </DragModal>
      )}
    </>
