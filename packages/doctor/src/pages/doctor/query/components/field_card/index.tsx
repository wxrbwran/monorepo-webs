...
import { DoctorSourceType, transformDynamicToStatic } from '@/pages/doctor/query/util';
...
function FieldCard({ currentField, onValueChange, type }: IProps) {
  ...
  const reloadDynamicItem = async (targetOption) => {
    ...
    const itemsRes = await transformDynamicToStatic(targetOption.items[0], doctorSid, doctorRoleSid, DoctorSourceType, targetOption.value, targetOption.parent);
  ...
  const onItemClick = (item, pageAt) => {
    if (item.isLeaf == false && item.children.length == 0) { // 说明子节点是动态的，并且没有获取过，需要请求
      ...
      if (item?.items?.length > 0 && item.items[0].type.includes('dynamic')) {
        ...
        transformDynamicToStatic(item.items[0], doctorSid, doctorRoleSid, DoctorSourceType, item.value).then((itemsRes) => {
