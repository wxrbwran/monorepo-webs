...
function Query({ }: IProps) {
  ...
  const fillChildItemsAndChildren = async (fatherItem) => {
    ...
    if (fatherItem.items?.length > 0) {
      ...
      if (fatherItem.items[0].type.includes('dynamic')) {
        // 登陆者的sid， 登陆者的角色
        const childItems = await transformDynamicToStatic(fatherItem?.items[0], doctorSid, doctorRoleSid, DoctorSourceType);
  ...
  const fetchField = async () => {
    ...
    const res = await api.query.fetchFields('CHRONIC_DISEASE_RESEARCH');
    let allItems = res.keys[0];
    const searchRangeTemp = allItems.items.filter((item) => item.name == 'team');
    if (searchRangeTemp.length > 0) {
      // 登陆者
      const items = await transformDynamicToStatic(searchRangeTemp[0], doctorSid, doctorRoleSid, DoctorSourceType);
      if (items.length > 0) {
  ...
  const handleSelectSearch = async (ruleItem, target?) => {
    ...
    if (getRuleItemType(ruleItem) == 'medical-img-noReference') {
      ...
      const res = await api.query.fetchKvScope({ kp: ruleItem.name.name, target, sourceType: DoctorSourceType });
