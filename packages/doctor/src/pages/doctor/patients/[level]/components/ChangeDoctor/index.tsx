  ...
  const fetchDoctorTeams = (roleId: string) => {
    ...
    api.doctor.getDotorTeams(roleId).then((res) => {
      ...
      res.teams.forEach((item: { members: ISubject[] }) => {
        ...
        if (filtermMember) { doctors.push(filtermMember); }
      });
                ...
                <div className={styles.doctor_list}>
                  {
                    doctorTeams.map((doctor) => (
                      <div
                        className={`${styles.doctor_item} ${relationDoctorId === doctor.wcId && styles.active}`}
                  ...
                  {
                    doctorTeams.length === 0 && <div className="flex justify-center w-full"><Empty /></div>
                  }
                </div>
              </div>
            )
