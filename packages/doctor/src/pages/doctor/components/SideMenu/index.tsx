        ...
        return (
          <SubMenu key="sub2" icon={<UserOutlined />} title="科室管理">
            {[...existedRoles].splice(1).map((item) => {
              ...
              const curDepHead = item.members.filter(
                (member) => member.role === Role.DEP_HEAD.id,
              )[0];
              return (
                <Menu.Item
                  ...
                  onClick={() =>
                    history.push(
                      `/doctor/dep_head?depHeadNsId=${curDepHead.nsId}&depHeadWcId=${curDepHead?.wcId}`,
                    )
                  }
                >
                  {curOrg.name}
    ...
    <div className={styles.side_menu}>
      ...
      >
        <SubMenu key="sub1" icon={<UserOutlined />} title="签约患者">
          {existedRoles?.[0]?.members
            .filter((item) => !!sidebar[item.role])
            .map((item: IExistedRole, inx: number) => {
              return (
                <Menu.Item
        ...
        <p>
          <Link to={'/publicize/patients'} className={styles.structure} rel="noopener noreferrer">
            <ProfileOutlined />
