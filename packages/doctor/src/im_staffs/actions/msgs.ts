...
export const onMsg = async(msg) => {
  ...
  if (msg.scene === 'team' && msg.type === 'notification') {
    ...
    // 被撤回的消息的msgId
    const delMsgId = JSON.parse(msg.custom)?.customMsgType<0 ? JSON.parse(msg.custom)?.msgId : '';
    if(contentId){
    ...
    if(delMsgId){
      const filterMsg = state.currSessionMsgs.filter(item => item.custom.msgId !== delMsgId);
      state.currSessionMsgs = [...filterMsg];
    }
    window.$store.dispatch({
      type: 'im/PUT_MSG',
      payload: msg,
...
export function updateCurrSessionMsgs(update_obj, state) {
  ...
  if (tmp_type === 'destroy') {
    ...
    if (newMsg) {
      ...
      state.currSessionMsgs=[...uniqueMsgs(newCurrSessionMsgs)];
