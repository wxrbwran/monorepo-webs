...
function SendPlan({ onCancel, infoIndex, addPlan, mode, plans, question, isDisabled }: IProps) {
  //起始发送时间默认值
  const initStart = {
    type: "START",
    detail: {
  ...
  const changeStateByValue = (value: string) => {
    ...
    treatment.detail.treatment = value;
    setSendTime(treatment);
  }
  //改变发送频率类型
  ...
  //子组件”发送条件“更改，回传回来的值
  const sendCondition = (plans:[]) => {
    setCallBackPlans(plans);
  }
  //发送实验组
  ...
  const disabled = mode === 'Add' ? !remind || isEmptyCustom || isEmptyGroup : isEmptyCustom || isEmptyGroup
  return (
    <div className={mode === 'Add' ? styles.send_plan : `${styles.send_plan} ${styles.edit}`}>
        ...
        <h2><span className={styles.start}>*</span>起始发送时间：</h2>
        <div className={styles.send_time}>
          <Select
            style={{ width: 180 }}
            onChange={handleChangeType}
            value={startTime.detail.start}
          >
            ...
            startTime.detail.start === 'TREATMENT_TIME' && (
              <div className={styles.item_value}>
                ...
                <Select
                  ...
                  // value={treatment}
                  onChange={(value: string) => { changeStateByValue(value) }}
                >
        ...
        <h2><span className={styles.start}>*</span>发送频率：</h2>
        <div className={styles.send_type}>
          <Select
            style={{ width: 180 }}
            onChange={handleGetType}
            value={frequency.detail.frequency}
          >
            {sendType.map(item => (
              <Option
                value={item.key}
                  ...
                  frequency.detail.custom.map((item: any, index) => (
                    <div className={styles.add_item} key={index}>
                      ...
                      <div className={styles.self_add}>
                        {index === 0 ?
                          <Button size="large" onClick={handleAddDayEdit}  >添加更多</Button> :
                          <Button size="large" onClick={() => handleDeleteDay(index)}  >删除</Button>
                        }
                      </div>
                    </div>
                  ))
        ...
        <h2><span className={styles.start}>*</span>发送试验组：</h2>
        <CheckboxGroup
          options={options}
