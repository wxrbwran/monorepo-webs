...
function Create({location}: IProps) {
  ...
  const delPlan = (index: number) => {
    const newInfos = infos.filter((item, vIndex) => vIndex !== index);
    const newStatus = status.filter((item, sIndex) => sIndex !== index);
    setInfos([...newInfos]);
  ...
  const handleSubmit = () => {
    ...
    if(!formName.trim()){
      ...
      console.log(99999)
      api.subjective.addObjectiveScale({
        infos,
        name: formName,
        type: 'OBJECTIVE',
        projectSid,
      ...
      <div className={styles.head}>
        ...
        <div className={styles.table_name}>
          <p className={styles.title}>
            <Input placeholder='请输入提醒类型，例：超声类检查提醒' onChange={changeFormName} style={{width: 400}}/>
          </p>
        </div>
      </div>
      <div className={styles.add} onClick={addInfo}><img src={create} alt=""/>创建新提醒</div>
      {
