          ...
          groupList.map((item, index) => {
            return (
              <div className={['item', currentGroupId === item.groupId ? 'active' : ''].join(' ')} key={item.groupName}>
                <Link to={`/patient_manage/group_details?groupId=${item.groupId}`} onClick={() => resetIndex(index)}>
                  <div className="flex justify-between pl-10 pr-20 group_item">
                    <span className="name">{item.groupName}</span>
                    <span>{item?.note?.note1}{item?.note?.note1 ? '人' : '--'}</span>
                    <span>{item?.patientCount || 0}人</span>
                  </div>
                </Link>
