...
function Reply({ location, scaleType }: IProps) {
  ...
  useEffect(() => {
    api.subjective.getScaleReplyList({
      ...
      if(res.subScaleReplyInfos.length>0){
        setDataSource(res.subScaleReplyInfos);
        let sendNum = 0;
        let replyNum = 0;
        res.subScaleReplyInfos.forEach((item:{sendNumber:number; replyNumber: number})=> {
