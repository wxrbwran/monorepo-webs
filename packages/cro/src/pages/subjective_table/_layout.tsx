...
function PatientManage(props: IProps) {
  ...
  const isShowSideMenu = !['/subjective_table/guide','/subjective_table/create', '/subjective_table/template'].includes(props.location.pathname); // 创建页面不显示侧边栏
  return (
        ...
        isShowSideMenu ? (
          <ToogleSide>
            ...
            <div style={{height: '100%'}}>
              {
                tableList.length > 0 && <ScaleTableTab id={props.location.query.id} location={props.location} scaleType="SUBJECTIVE" />
              }
              {props.children}
            </div>
          </ToogleSide>
