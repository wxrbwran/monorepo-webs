...
import { fetchRolePropValue, Role } from 'xzl-web-shared/dist/src/utils/role';
    ...
    >
      {
        data.name ?
          <ViewDetails
            data={data}
            style={{ 'flex': '0 0 auto', 'marginRight': 38 }}
            refresh={refresh}
          >
      ...
      >
        <>
          ...
          <span className={styles.flex3}>
            {
              data?.subjectDetail?.practiceAreas && data.subjectDetail.practiceAreas.map((item) => {
                return (
                  <div>
                    {
