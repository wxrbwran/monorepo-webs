...
function GroupStatic({ projectSid }: IProps) {
  ...
  useEffect(() => {
    if (projDetail.detail) {
      ...
      const infoArr = [joinStandard, excludeStandard].filter((item) => !!item);
      setInfos([...infoArr]);
      if (infoArr.length !== 0) {
        ...
        <div className={styles.submit}>
          {isShowEdit && infos.length !== 0 ? (
            <div className={styles.detail}>
              <Button type="primary" ghost onClick={() => setIsShowEdit(false)}>
                编辑
              </Button>
            </div>
          ) : (
            <>
              ...
              <Button disabled={!isLeader} onClick={() => handleSave(1)}>
                保存
              </Button>
            </>
          )}
          ...
          <div key={index}>
            ...
            <ul className={styles.main}>
              ...
              {[0, 1].includes(+item.gender) && (
                <li>
                  {findIndex(item, 'gender')}. <b>性别:</b>{' '}
                  <p className={styles.value}>{sexList[+item.gender]}</p>
                </li>
              )}
              {item.medicineName && (
                <li>
                  {findIndex(item, 'medicineName')}. <b>用药:</b>{' '}
                  <p className={styles.value}>{item.medicineName.join()}</p>
                </li>
              )}
              {item.diagnoseName && (
                <li>
                  {findIndex(item, 'diagnoseName')}. <b>诊断:</b>{' '}
                  <p className={styles.value}>{item.diagnoseName.join()}</p>
                </li>
              )}
              {item.customize && (
                <li>
                  {item.customize.map((custo: string, idx) => (
                    <p className={styles.customize}>
                      {findIndex(item, 'customize') + idx}. {custo}
                    </p>
                  ))}
                </li>
              )}
