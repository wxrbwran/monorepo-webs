...
function QueryDetail({ children, row, showModal, onCancel }:IProps) {
  ...
  useEffect(() => {
    if(showModal){
      api.query.fetchQueryDetail({ kp: row.kp, ids: row.source }).then((res) => {
        const results = res?.results;
        if(results.length > 0){
          setTitleHead(results[0]?.titleHead);
          setDataSource(results[1]?.cells);
          setImgList(results[1]?.cells.map((item: { url: string; }) => item?.url).filter((i: string) => !!i));
          const formatResult = results[1]?.columns.map((item: IColumns) => ({
            ...item,
            render: (text: any, record: IColumns) => {
