...
function Define() {
  ...
  const handleSave = (values: CommonData) => {
    ...
    if(eventDetail?.id){ params.id = eventDetail?.id}
    api.event[serviceApi]({...params}).then((res) => {
  ...
  const fetchEndEvent = () => {
    api.event.fetchEndEvent(projectSid).then((res) => {
      setEventDetail(res);
  ...
  const sick = ['死亡', '危及生命', '永远、严重残疾、功能丧失', '先天性异常、出生缺陷', '需要住院治疗、延长住院时间']
  return (
    <div className={styles.define}>
      {isDraft && !isEdit && !isEmpty(val) && (<img src={draft} className={styles.draft_img}/>)}
      <Form
        name="event"
      ...
      >
        ...
        <Form.Item>
          <Button htmlType="submit" onClick={()=> setIsDraft(false)}>保存</Button>
        </Form.Item>
      </Form>
      {
        !isEdit && (
          <div className={styles.detail}>
            ...
            />
            {
              window.$storage.getItem('isLeader') && isDraft && window.$storage.getItem('projectStatus') != 1001 &&(
              <Button type="primary" ghost onClick={handleEdit}>编辑</Button>
              )
            }
          </div>
        )
