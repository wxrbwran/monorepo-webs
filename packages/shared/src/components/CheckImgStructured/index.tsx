...
const CheckImgStructured: FC<IProps> = (props) => {
  ...
  const fetchImageJcds = async (imageId: string, groupId: string | undefined) => {
    ...
    const data = window.$api.image.fetchImageJcdAndOther(params);
    return data;
  };
  const fetchImageIndexes = async (imageId: string, groupId: string | undefined) => {
    ...
    const data = await window.$api.image.fetchImageIndexes(params);
    return data;
  };
  ...
  return (
    <>
      ...
      <DragModal
        ...
        destroyOnClose
      >
        <div className='shared-check-img-structured'>
          {!isEmpty(imgData) ? (
