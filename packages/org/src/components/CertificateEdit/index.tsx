...
function CertificateEdit(props: IProps) {
  ...
  const titleObj: Store = {
    qualification: '医师执业证书',
    profession: '医师资格证书',
    badge: '手持工牌照片',
  };
  const [certificateArr, setCertificateArr] = useState<Store>(imgList);
  useEffect(() => {
    const newCerArr: Store = { ...certificateArr };
  ...
  const imgSuccess = (type: string, imgIndex: number, imgUrl: string) => {
    const newCertificates: Store = { ...getFieldValue('certificates') };
  ...
  const handleImageRotate = (degreeNumber: number) => {
    const count = parseInt((Math.abs(degreeNumber) / 360).toString(), 10) + 1;
    /* eslint-disable operator-linebreak */
    const degree: number =
      degreeNumber < 0 ? degreeNumber + count * 360 : degreeNumber;
...
CertificateEdit.defaultProps = { certificates: {} };
