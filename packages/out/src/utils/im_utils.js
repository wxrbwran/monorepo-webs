...
Utils.generateTeamSysmMsg = function (data) {
  var text,
    nicks = this.getNickNames(data.attach.users);
  switch (data.attach.type) {
    case 'updateTeam':
...
// todo 写成私有成员方法
Utils.getNickNames = function (users, state) {
  return users.map((user) => {
    return user.account === window.$storage.getItem('uid') ? '你' : user.nick;
  });
};
...
Utils.getTeamUpdateInfo = function (msg) {
  ...
  // }
    ...
    text = `群身份验证模式更新为${
      team.joinMode === 'noVerify'
        ? '不需要验证'
        : team.joinMode === 'needVerify'
        ? '需要验证'
        : '禁止任何人加入'
    }`;
 ...
 * 输入Unix时间戳，返回指定时间格式
 */
Utils.calcTimeHeader = function (time) {
  // 格式化传入时间
