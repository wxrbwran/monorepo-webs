...
const NSLabelType = {
  ...
  CHRONIC_DISEASE_TEAM_SESSION: fetchData(
    'session_group',
    '团队成员内部会话组_SPACE',
  ), // 15

  COMMON_DEPARTMENT_TYPE: fetchData('common_department_type', '通用科室'), // 16
  CARDIOVASCULAR_DEPARTMENT_TYPE: fetchData(
    'cardiovascular_department_type',
    '心血管科',
  ), // 17
  ...
  RESEARCH_PATIENT_INVITE: fetchData(
    'research_patient_invite',
    '科研项目患者邀请空间',
  ), // 22
  RESEARCH_PROJECT_PATIENT: fetchData(
    'research_pro_patient',
    '科研项目受试者空间',
  ), // 23
  RESEARCH_PATIENT_GROUP: fetchData(
    'research_patient_group',
    '科研项目患者小组空间',
  ), // 24
  RESEARCH_ORG: fetchData('research_org', '科研项目机构空间'), // 25
  RESEARCH_DEP: fetchData('research_dep', '科研项目科室空间'), // 26
  RESEARCH_PROJECT_DOCTOR: fetchData(
    'research_pro_doc',
    '科研项目成员医生空间',
  ), // 27
