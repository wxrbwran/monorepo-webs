...
const EducationCreate: FC<ILocation> = ({ location }) => {
  ...
  useEffect(() => {
    api.education.getRules().then((res) => {
  ...
  const onFinish = (values: IValues) => {
    ...
    if(result){
      ...
      api.education.sendPlan(params).then(() => {
  ...
  const handleChange = (changedValues: any, allValues: { conditions: {type: string}[];}) => {
    ...
    if (vals.includes("age")) { disableObj.age = true }
    if (vals.includes("sex")) { disableObj.sex = true }
  ...
  return (
    ...
    >
      <div className={`${styles.edu_create} p-20`}>
