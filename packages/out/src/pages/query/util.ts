...
export const transformDynamicToStatic = (item: { type: string, name: string, assign: { value: string } }, doctorSid: string, doctorRoleId: string, doctorNsId: string, orgSid: string, orgRoleId: string, sourceType: string, value?: string, fatherValue?: string) => {
  ...
  return new Promise<any[]>((resolve, reject) => {
    ...
    if (item.type.includes('dynamic')) {
      // 动态加载
      const params: { kp: string, value?: string, rsList: { sid: string, roleType?: string, nsId: string }[], scaleType?: string, sourceType: string, fatherValue?: string } = {
        kp: item.name,
        value: value,
        //// 登陆者 sid,   roleType 登陆者角色
        // 院外 登陆者 sid,   roleType 登陆者角色  nsId: 登陆者nsid  operator  和机构的sid，机构角色 nsId: 机构nsid  ownership
        rsList: [
          { sid: doctorSid, roleType: doctorRoleId, nsId: doctorNsId },
          { sid: orgSid, roleType: orgRoleId, nsId: doctorNsId },
        ],
