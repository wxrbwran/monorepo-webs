...
import addPng from '@/assets/img/query/add.png';
import deleteBluePng from '@/assets/img/query/delete_blue.png';
import copyPng from '@/assets/img/query/copy.png';
...
function Query({ }: IProps) {
  ...
  const fillChildItemsAndChildren = async (fatherItem) => {
    ...
    if (fatherItem.items?.length > 0) {
      ...
      if (fatherItem.items[0].type.includes('dynamic')) {
        const childItems = await transformDynamicToStatic(fatherItem?.items[0], doctorSid, doctorRoleSid, doctorNsId, orgSid, orgRoleId, DoctorSourceType);
        fatherItem.items = [...childItems];
      }

      if (fatherItem.children == undefined) {
        // 将所有数据都添加上formateData
        const tempData = fatherItem?.items?.filter(f => !!f.show).map((itemTemp: any, index: number) => {
          ...
          return (
            {
              ...
              children: itemTemp?.items ? itemTemp.items?.filter(t => !!t.show).map((childItem: any, idx: number) => ({
  ...
  const fetchField = async () => {
    ...
    const res = await api.patient_query.fetchFields('CHRONIC_DISEASE_RESEARCH');
    let allItems = res.keys[0];
    const searchRangeTemp = allItems.items.filter((item) => item.name == 'team');
    if (searchRangeTemp.length > 0) {
      // 登陆者
      const items = await transformDynamicToStatic(searchRangeTemp[0], doctorSid, doctorRoleSid, doctorNsId, orgSid, orgRoleId, DoctorSourceType);
      if (items.length > 0) {
  ...
  const options = searchRangeItems.map((item: any) => ({
  ...
  const handleSelectSearch = async (ruleItem, target?) => {
    ...
    if (getRuleItemType(ruleItem) == 'medical-img-noReference') {
      ...
      const res = await api.patient_query.fetchKvScope({ kp: ruleItem.name.name, target, sourceType: DoctorSourceType });
  ...
  const onNextStepClick = async () => {
    ...
    if (currentStep == 2) { // 构造出来的查询条件需要
      ...
      if (rules) {
        ...
        const head = await api.patient_query.fetchQueryId(params);
        dispatch({
          type: 'patient_query/setQueryHead',
