...
const ChatItem: FC<IMsg> = (props) => {
  ...
  if (item.type === 'text') {
    ...
    if (Array.isArray(item.content?.content)) {
      item.showText = item.content?.content.filter((con) => [
  ...
  return (
    ...
    >
      {/* 103 调整治疗方案  123app调整用药 119 项目邀请 */}
      {[103, 123].includes(msg.content?.type)
      ...
      {(msg.flow === 'in' || msg.flow === 'out') && !['tip', 'custom'].includes(msg.type) && (
        <div className="chat__item">
          ...
          <div className="chat__item-content">
            ...
            {msg.type === 'text' && <p className="msg-text">{msg.showText}</p>}
            {msg.type === 'image' && (
              <p
            ...
            {msg.type === 'video' && (
              <p className="msg-text video" ref={mediaMsg} />
            )}
            {msg.type === 'audio' && (
              <p
