...
function SecurityLayout({ children, location }: IProps) {
  const excludeRouter = ['patient_panel', 'login', 'find_pwd', 'structured', 'checked_img']; // 不显示导航的页面
  const excludeCommonTabRouter = ['login', 'find_pwd', 'patients']; // 不显示详情tab的页面
  ...
  return (
    <div className={styles.app}>
      <>
        ...
        {!excludeCommonTabRouter.includes(pathname) && <CommonTab />}
        {children}
        <ImWindow />
      </>
    </div>
